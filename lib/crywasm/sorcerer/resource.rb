...
module CryWasm
  module Sorcerer
    class Resource
      ...
      def resource(sexp)
        raise NotSexpError, "Not an S-EXPER: #{sexp.inspect}" unless sexp?(sexp)

        handler = HANDLERS[sexp.first]
        raise NoHandlerError, sexp.first unless handler
      ...
      def opt_parens(sexp)
        emit(' ') if !params_have_parens?(sexp) && !params_are_empty?(sexp)
        resource(sexp)
      end
      ...
      def quoted_word_add?(sexp)
        sexp &&
          sexp[1] &&
          %i[words_add qwords_add qsymbols_add symbols_add].include?(sexp[1].first)
      end

      def quoted_word_new?(sexp)
        sexp &&
          sexp[1] &&
          %i[qwords_new words_new qsymbols_new symbols_new].include?(sexp[1].first)
      end

      def words(marker, sexp)
        emit("%#{marker}{") if @word_level == 0
        @word_level += 1
        unless quoted_word_new?(sexp)
          resource(sexp[1])
          emit(' ')
        end
        resource(sexp[2])
      ...
      def determine_regexp_delimiters(sexp)
        sym, end_delim, = sexp
        raise UnexpectedSexpError, "Expected :@regexp_end, got #{sym.inspect}" unless sym == :@regexp_end

        end_delim_char = end_delim[0]
        first_delim = BALANCED_DELIMS[end_delim_char] || end_delim_char
        first_delim = "%r#{first_delim}" if first_delim != '/'
      ...
      NOOP = ->(sexp) {}
      SPACE = ->(_sexp) { emit(' ') }
      PASS1 = ->(sexp) { resource(sexp[1]) }
      PASS2 = ->(sexp) { resource(sexp[2]) }
      PASSBOTH = lambda { |sexp|
        resource(sexp[1])
        resource(sexp[2])
      }
      EMIT1 = ->(sexp) { emit(sexp[1]) }
      ...
      HANDLERS = {
        ...
        :module => lambda { |sexp|
          ...
          resource(sexp[1])
          newline
          indent { resource(sexp[2]) } unless void?(sexp[2])
          emit('end')
        },
        ...
        :when => lambda { |sexp|
          ...
          newline
          resource(sexp[2])
          emit(' ') if sexp[3] && sexp[3].first == :when
          resource(sexp[3]) if sexp[3]
        },
        :while => lambda { |sexp|
          emit('while ')
